...
| 6 | **형상관리** | Ops | 검수 + todo 갱신(spine + `docs/todo.md` 채택 시 그쪽도) + PR + merge | ✓ | ✓ | ✓ |
| 7 | **완료** (평가·개선·다음 작업 제안) | Orchestrator | §10.1 지표 확정 · §10.3 기준 개선 제안 · 다음 작업 제안(입력 = spine의 `다음 행동` + `docs/todo.md` 채택 시 그쪽 `다음 관문`) | 기록 1줄 | ✓ | ✓ |
...
- **계기 규율 — 경로를 세는 검증**: 파일 경로를 세거나 경로 집합을 대조할 때 `git ls-files`·`ls-tree`·`status --porcelain`의 **텍스트 출력을 줄·공백으로 쪼개지 않는다**. git은 비ASCII 경로를 **C-quote(8진 이스케이프)** 하고 공백 경로는 한 줄 안에서 갈라져, `.split()`과 `.splitlines()`가 **서로 다른 이유로 조용히 놓친다**(exit 0·경고 없음). 처방은 **`-z` + NUL 분리** 하나뿐이다(`check-ignore --stdin`은 입·출력 양쪽에 `-z`). ⚠️ 지금 그런 경로가 없는 repo도 **잠복이지 부재가 아니다** — 발현율은 리포마다 갈린다(평균으로 재면 한글 명명 관행이 있는 repo를 과소평가한다). ⚠️ **이 규칙은 파싱만 다룬다** — "계기가 애초에 동작했나"(부재 판정의 양성 대조군)는 **별개 축이고 아직 규범이 아니다**(후보로 보류). `-z`만 쓰고 대조군이 없으면 여전히 계기 실패를 못 잡는다.
...
- 돌린 오라클: <**요지 1~2줄**, 상세는 raw — **경로를 세는 명령은 `-z` 사용 여부 표기**(§4 계기 규율 · §10.2 집계 지점)>
```
...
- `artifacts/progress/todo.md`(spine) 갱신 — 항목별 1줄: `status(open/tried/passed/blocked) · 다음 행동 · 검증 증거 링크 · 되돌릴 커밋`. 상세 증거는 artifacts/review·tests 링크로(todo 오염 방지). spine = **작업 단위 원장**(독자=그 작업을 하는 사람, 질문="검증됐나·되돌리려면 어느 커밋인가").
  - **spine이 소유하지 않는 것** — `docs/todo.md`(project-init 상태문서 — 채택 선택)를 둔 프로젝트면 **현재 위치·다음 관문·열린 결정**은 그쪽 소유이고(독자=다음 세션 진행자, 질문="지금 어디고 다음에 뭘 하나") **세션 재개 첫 Read = `docs/todo.md`**. 없으면 spine이 재개 진입점을 겸한다. 🔴 **서로 복사하지 말고 링크한다** — 복사하는 순간 원장이 둘이고, 갈리면 어느 쪽을 믿을지가 다음 분쟁이 된다. ⚠️ `docs/todo.md`는 **4절 고정**(현재 위치·다음 관문·열린 결정·진행 로그 — 글로벌 `project-init` 규정 "여기서 늘리지 마라")이라 **항목별 `다음 행동`을 담을 슬롯이 없다**. 그건 분기와 무관하게 **spine이 계속 소유한다**.
...
7단계는 기록에 그치지 않는다 — §10.3 기준으로 즉시 개선점을 제안하고(작업 단위 마이크로 평가), **다음 작업을 제안**한다 — 입력은 spine의 `다음 행동`과, `docs/todo.md`를 둔 프로젝트면 그쪽 `다음 관문`.
...
- **todo 소유 경계 준수(§9 DoD)** — `docs/todo.md`를 둔 프로젝트에서 **두 문서에 같은 사실이 복사된 건수(목표 0)**(현재 위치·다음 관문·열린 결정이 spine에도 적힌 경우) ↔ **counter-metric: 링크만 두어 세션 재개가 실패한 건수**(재개 시 필요한 정보에 한 번에 못 닿음). 개선 없으면 §10.3으로 폐기 검토
- **경로 계기 규율 준수(§4)** — **산출물 위치 = §6.7 Summary의 `돌린 오라클` 필드**(상세는 raw 리뷰 파일). 거기 적힌 **경로를 세는 명령 중 non-`-z` 사용 건수(목표 0)** ↔ **counter-metric: 경로 대조 자체를 생략한 리뷰 건수**(`-z`가 번거로워 아예 안 세는 쪽으로 도망간 경우). ⚠️ **"거짓 부재 사건 수"는 지표로 쓰지 않는다** — 발견되지 않은 것은 정의상 셀 수 없어 항상 0을 보고하고(자기승인 지표), 그걸 잡으려면 §4가 **미규범으로 보류**한 양성 대조군 축이 필요하다. 개선 없으면 §10.3으로 폐기 검토
...
| `artifacts/progress/todo.md` | **작업 단위 원장**(spine) — 항목 status · 검증 증거 · 되돌릴 커밋 + Fast-track `[FT]` 마킹. **`docs/todo.md`를 둔 프로젝트면** 현재 위치·다음 관문·열린 결정은 그쪽 소유(복사 금지·링크만) + 재개 첫 Read도 그쪽, **없으면** spine이 그것까지 겸한다. ⚠️ 항목별 `다음 행동`은 `docs/todo.md`에 슬롯이 없어 **분기와 무관하게 spine 소유** — 분기 정본 = §9 DoD |
...
- **2026-08-10 todo 소유 경계 + 경로 계기 규율 (2건 묶음 — rtong 승인)**: (1) **todo 소유 경계** — 글로벌 `project-init`에 상태문서 3종(`docs/todo.md` 정본 + `docs/todo.html`·`dashboard.html` 뷰)이 표준으로 들어오면서(claude-config PR #198, **소급 강제 아님**), 그 표준으로 시작하고 본 템플릿을 쓰는 신규 프로젝트는 **todo 정본을 둘** 갖게 됐다("다음 관문"↔"다음 행동", "진행 로그"↔"status" 중복). **무증상일 때** 경계를 물었고(직전 층 소유 건과 동일 프로필 — 실제로 쓰는 순간 갈린다), claude-config 답 = **계층 분리**이되 판별식은 "무엇을 적나"가 아니라 **"독자가 누구고 무슨 질문에 답하나"**: `docs/todo.md`(독자=다음 세션 진행자 / "지금 어디고 다음에 뭘 하나") ↔ spine(독자=그 작업을 하는 사람 / "검증됐나·되돌리려면 어느 커밋인가"). 반영: **§9 DoD**에서 **소유와 재개 진입점만** `docs/todo.md` 유무로 갈리게 하고(spine 필드는 분기와 무관하게 `status·다음 행동·검증 증거·되돌릴 커밋` 동일 — `docs/todo.md`는 4절 고정이라 항목별 `다음 행동` 슬롯이 없다) §11 산출물 표를 **"작업 단위 원장"**으로 재서술 — 🔴 **복사 금지·링크만**(복사하면 원장이 둘). `docs/todo.md`를 안 두는 프로젝트는 spine이 재개 진입점을 겸한다(조건절 — 채택이 선택이라 무조건문이면 거짓이 된다). 상대측도 자기 `§5-1`에 spine 대응 줄을 넣어 **양쪽 정본이 각자 소유하지 않는 것을 명시**하는 형태. (2) **경로를 세는 검증의 계기 규율(§4)** — `git ls-files`·`ls-tree`·`status --porcelain`의 텍스트 출력을 줄·공백으로 쪼개면 **`.split()`과 `.splitlines()`가 서로 다른 이유로 조용히 놓친다**(공백 경로 / 비ASCII의 **C-quote 8진 이스케이프**, `core.quotePath` 기본 true) — exit 0·경고 없음. 처방 = **`-z` + NUL 분리**. 근거: claude-config 통지·정정(dakman-atrium 프로브 실측 **경유** — 우리가 1차로 재현하지 않았다) + **우리 repo 자체 측정**(추적 50건 중 비ASCII 0건 = 세 파서 일치하나 이는 **트리거 부재이지 파서 정확성이 아니다** — 잠복). 이 규율의 직접 계기는 본 세션에서 회수 검증에 쓴 명령들(`ls-tree -r --name-only`·`status --porcelain`·`log --name-only`)이 전부 그 면이었다는 자기 점검이다. **범위 판단**: "부재 판정엔 양성 대조군" 확장은 승인 범위 밖이라 제외했는데, **held-out 3인이 "그러면 규칙이 자기 관할의 절반만 처방한다"고 반론했고 그 반론이 성립한다** — 규칙의 *처방*을 넓히는 대신 *관할*을 좁혀 해소했다(파싱만 다루고, "계기가 애초에 동작했나"는 별개 축·미규범으로 명시). 양성 대조군은 **후보로 보류**. **동기화**: `docs/project-init.md` 폴더 구조 주석에도 경계를 반영. ⚠️ **R2에서 `docs/project-init.html` 미러가 stale임이 드러났다** — `.md`는 이번·직전(2026-07-27) 변경을 받았는데 `.html`은 그 이전 커밋에 멈춰 있었다(`{중앙DS}`·`{E2E실행}`·`{실행환경}`·층 소유 전부 0건). 즉 **직전 항목의 "html 동기화" 기록은 workflow 쌍만 참이었다** — project-init 쌍까지 이번에 함께 맞췄다. 교훈: 미러 쌍은 *한 쌍*이 아니라 **repo 안 모든 md↔html 쌍**이다. **메타 = 자동 T3 + held-out 3인**: R1 전원 FAIL(codex C0M1·grok C0M4·claude C0M5) → R2(codex C0M2·grok PASS·claude C0M2)에서 **교정이 만든 신규 결함 2건**을 더 잡았다 — (a) 내가 `다음 행동`을 `docs/todo.md` 소유로 옮겼는데 **그 문서엔 슬롯이 없다**(글로벌 규정 "4절만·여기서 늘리지 마라" + `build-dashboard.py` 절 이름 하드바인딩 — 1차 원문 확인). claude-config 답변에도 `다음 행동`은 없었고 **내가 임의로 넣은 것**이라, 분기와 무관하게 spine 소유로 되돌렸다. (b) §10.2 계기 지표가 **산출물 위치가 없고** "거짓 부재 사건 수"는 **정의상 셀 수 없어 항상 0을 보고**하는 자기승인 지표였으며, 무엇보다 §4가 **미규범으로 보류**한 양성 대조군 축에 의존해 **같은 개정 안에서 모순**이었다 → 위치(`artifacts/review/` 오라클 기록)·자기검출 가능한 계수(non-`-z` 사용 건수)로 교체하고 거짓 부재 항목은 삭제. (c) **`docs/project-init.html` 미러가 PR #23 이후 stale**이었다(아래 참조). R3(codex C0M1·grok C0M1·claude C0M3)은 **같은 실패 유형의 재발**을 잡았다 — (a)의 교정이 `docs/project-init.{md,html}`엔 전파되지 않아 두 파일이 여전히 `다음 행동`을 `docs/todo.md` 소유로 적었고(html 동기화 교정이 그 결함을 **두 번째 파일로 복제**했다), 더 나쁘게 **§10.2 지표의 산출물 위치로 지정한 "돌린 오라클" 슬롯 자체가 존재하지 않았다**(전수 히트 1 = 그 문장 자신). 즉 *슬롯 없는 곳을 정본이 가리킨다*는 이번 개정이 고치려던 바로 그 결함을, 고치는 커밋 안에서 두 번 저질렀다. 처분: project-init 두 파일 정정 + §12 자기모순 문장 정정 + **rtong 승인으로 R4를 열어 §6.7 Summary에 `돌린 오라클` 필드를 신설**해 지표가 실재하는 자리를 가리키게 했다(슬롯을 안 만들면 §10.3 "측정 지표 존재" 관문을 못 넘어 규칙 자체가 성립하지 않는다). R1 지적은 수렴했다: (1) DoD 부모 불릿이 `다음 행동`·"세션 재개 첫 Read"를 **무조건 소유**해 새 조건절과 정면 충돌 → **소유와 재개 진입점만** `docs/todo.md` 유무로 갈리게 정리(spine 필드 자체는 분기와 무관하게 동일 — R2에서 한 번 더 좁혔다), §3 6·7단계·§10.1·§11 포함 (2) 새 문안이 DoD를 **`§3`으로 오참조**(실체는 §9) (3) §10.3 "측정 지표 존재" 관문 미통과 → §10.2에 2줄 배선(직전 메타 개정들이 전부 배선한 전례와 어긋났다) (4) 위 관할 반론 (5) `project-init` 미동기화.
