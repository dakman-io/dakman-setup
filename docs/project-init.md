...
>
> 🤖 이 절차는 **`project-init` 스킬**(글로벌 단일 정본 `~/.claude/skills/project-init/` — 어느 디렉터리에서나 발동, 리포에 사본을 두지 않음)로 실행할 수 있다 — "새 프로젝트 시작"이라고 하면 발동. 본 문서는 그 스킬의 정본 설계 참조이며, 비가역 원격 작업(저장소 생성·브랜치 보호·main 삭제)은 스킬이 실행 계획을 보여주고 승인받은 뒤 처리한다.
>
...
## 크로스 프로젝트 작업 경계 (멀티 세션 규약)

각 프로젝트 세션은 **자기 repo만 직접 작업**한다. 자기 프로젝트 밖(다른 `~/<도메인>/workspace/<repo>`)의 파일·작업은 — git뿐 아니라 **편집·생성·ingest 등 작업 자체를** — 직접 하지 말고, 그 프로젝트의 **cmux 진행자에게 `cross-project-notify`로 위임(요청)**한다. 위임 시 **회신처(자기 workspace/surface)를 첨부**해, 작업을 받은 세션이 완료하면 회신하게 한다(양방향 — 닫힌 루프).
...
- **말투**: 대화는 반말로 한다.
- **업무 응답 형식**: 업무 대화·답변·보고는 서술형(줄글) 금지 — 헤더·불릿·표 중심의 **보고서 형태**로 응답한다. (가벼운 잡담만 자연스러운 대화체)
...
2. **§0 placeholder 채우기** — workflow.md §0 표의 프로젝트별 값을 채운다: `{프로젝트명}`·`{플랫폼}`·`{타입체크}`·`{단위테스트}`·`{린트}`·`{E2E도구}`·`{E2E실행}`·`{디자인도구}`·`{중앙DS}`·`{실행환경}`·`{커버리지}`·`{위험도메인}`·`{AI예산}`. **`{위험도메인}`은 특히 명시적으로** — 실패 시 데이터·돈·신뢰에 직접 영향 주는 핵심 로직 (자동 T3 입력값). **`{중앙DS}`도 시작 시 확정** — 소비하는 중앙 디자인시스템 repo·판(`없음`/`본체` 포함). 원칙 8 층 소유의 입력값이라 나중에 읽는 시점의 자기선언으로 정하면 안 된다.
3. **핵심 원칙 정합 확인** — 본 프로젝트의 방법론이 workflow.md 원칙과 일치하는지 점검: **SDD**(원칙 1, 문서→디자인→소스) · **ATDD+TDD 이중 루프**(원칙 3) · **디자인 시스템 4 레이어 + 층 소유**(원칙 8 — `{중앙DS}` 값이 정함: repo·판이면 Foundation·Component=중앙 DS / Pattern·Screen=제품 · `없음`=제품 4층 · `본체`=이 repo가 정본) · **Tier 분류**(§2). 상세 정의는 workflow.md를 정본으로 참조.
