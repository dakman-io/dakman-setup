...
**3축 yes/no 평가** (30초 안에 판단). 의심되면 한 단계 상향.

| 축 | 질문 |
|---|------|
| **사용자 영향** | 사용자가 직접 보거나 체감하는가? |
| **되돌림 비용** | 실패 시 되돌리기 어렵거나 데이터 영향이 있는가? |
| **검증 가능성** | 자동 테스트 또는 명확한 수동 확인으로 증명 가능한가? |

| Tier | 정의 | 예시 |
|------|------|------|
| **T1 Trivial** | 영향 낮음 + 되돌림 쉬움 + 검증 명확 | 오탈자, lint, 포맷팅, unused import, 기존 expectation 보정 |
| **T2 Standard** | 3축 중 1개라도 중간 이상 | 기존 US 1개 구현, UI 텍스트, 컴포넌트 추가 |
| **T3 Major** | 영향 높음 + 되돌림 어려움 함께 또는 검증 불명확 | 신규 US, 데이터 모델/마이그레이션, `{위험도메인}`, 메타 워크플로우 |
...
**처리**:
- 3회 이상 우회된 규칙 → 삭제·축약·예외화
- 3회 이상 즉흥 처리된 노하우 → 문서화
- 새 규칙 "운영 가능성 체크" 통과 필수: 30초 판단 가능 / 산출물 위치 명확 / 생략 조건 명확 / 중복 X
